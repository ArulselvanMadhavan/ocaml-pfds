...
module Exercise3_4 (E : ORDERED) = struct
  ...
  let rec merge = function
    ...
    | (T (_, _, x, _, _) as h1), (T (_, _, y, _, _) as h2) ->
      if E.leq (x, y)
      then check_weight_and_build x h1 h2
      else check_weight_and_build y h2 h1

  and check_weight_and_build elem (T (lsize, rsize, _, l, r)) h2 =
    let merged_weight = rsize + weight h2 in
    if lsize >= merged_weight
    then T (lsize, merged_weight, elem, l, merge (r, h2))
    else T (merged_weight, lsize, elem, merge (r, h2), l)
  ;;
end
